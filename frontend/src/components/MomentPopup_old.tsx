...
export default function MomentPopup({ media, onClose, refreshMoments }: Props) {
  ...
  useEffect(() => {
    const saveTimeout = setTimeout(() => {
      ...
      setSaving(true);
      axios
        .patch(`http://localhost:5000/api/moments/${media._id}`, { description }, {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/media/${media._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      onClose();
      refreshMoments();
  ...
  const toggleFavorite = async () => {
    try {
      await axios.patch(`http://localhost:5000/api/media/favorite/${media._id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      >
        ...
        <div className="mt-8">
          {media.fileUrl.match(/\.(mp4|webm|ogg)$/i) ? (
            <video src={media.fileUrl} controls className="w-full rounded-lg max-h-[50vh] object-cover" />
          ) : (
